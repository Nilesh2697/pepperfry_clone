  ...
  const body = (
    <div style={modalStyle} className={classes.paper}>
      <div>
        ...
        />
        {!isResetLoading ? (
          <p style={{marginLeft:25,color:"green"}}>{isMessage}</p>
        ) : isError ? (
          <p style={{marginLeft:25,color:"green"}}>{isMessage}</p>
        ) : null}
        ...
        <div style={{ marginLeft: 30 }}>
          <p style={{ fontSize: 15, marginTop: 12, color: "black" }}>
            Just remembered?{" "}
            <span style={{ color: "#ef6630",cursor:"pointer" }} onClick={handleToggleToLogin}>
              Log In
            </span>{" "}
            Instead
          </p>
          <p style={{ color: "black", fontSize: 15,marginBottom:15 }}>
            Don't have an account?{" "}
            <span style={{ color: "#ef6630" ,cursor:"pointer"}} onClick={handleToggleRegister}>
              Register with us
            </span>
          </p>
        </div>
